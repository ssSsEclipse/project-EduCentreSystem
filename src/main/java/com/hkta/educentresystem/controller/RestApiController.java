...
public class RestApiController {
	...
	public ResponseEntity<CentreDto> getCentre(@RequestParam(value = "id", required = false) Long id, @RequestParam(value = "couponCode", required = false) String couponCode) {
		Centre centre = null;
		if (id != null) {
			centre = centreService.findOne(id);
		} else if (!StringUtils.isEmpty(couponCode)) {
			centre = centreService.findByCouponCode(couponCode);
		}
		if (centre != null) {
			CentreDto centreDto = dozerMapper.map(centre, CentreDto.class);
			return ResponseEntity.status(HttpStatus.OK).body(centreDto);
		}
