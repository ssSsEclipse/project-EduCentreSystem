...
public class TransactionController {
	...
	public TransactionListDto listTransactions(
		...
		if (centre != null) {
	        Page<Transaction> resultPage = transactionService.findByCentreWithMonthYear(centre.getId(), month, year, page, size);
	        if (page > resultPage.getTotalPages()) {
	            throw new ResourceNotFoundException();
	        }
	        result.setResult(resultPage);
	        
	        Page<Transaction> allResults = transactionService.findByCentreWithMonthYear(centre.getId(), month, year, 0, Integer.MAX_VALUE);
